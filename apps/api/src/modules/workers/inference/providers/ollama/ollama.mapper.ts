// apps/api/src/modules/workers/inference/providers/ollama/ollama.mapper.ts
...
export class OllamaMapper {
  static toChatRequest(request: InferenceRequest): OllamaChatRequest {
    ...
    return {
      ...
      // THE FIX: Map keepAlive, fallback to extraOptions, default to -1 (permanent VRAM lock)
      keep_alive:
        request.keepAlive ??
        (request.extraOptions?.keep_alive as string | number) ??
        -1,
  ...
  static toInferenceResponse(response: OllamaChatResponse): InferenceResponse {
    ...
    // Support newer Ollama reasoning models (e.g. DeepSeek-R1) that split out the thinking
