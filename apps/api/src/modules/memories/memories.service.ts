...
export class MemoriesService {
  ...
  async update(userId: string, memoryId: string, data: UpdateMemoryData) {
  ...
  async updateLastAccessed(userId: string, memoryId: string) {
    await this.findById(userId, memoryId);

    return this.memoriesRepository.updateLastAccessed(memoryId);
  }
  ...
  async searchSimilar(userId: string, query: string, limit = 5) {
    return this.memoryIndexService.searchSimilar(userId, query, limit);
  }
}
