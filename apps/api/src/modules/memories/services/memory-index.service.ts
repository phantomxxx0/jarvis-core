...
export class MemoryIndexService {
  ...
  async index(memory: any): Promise<void> {
    try {
      const embedding =
        await this.aiRouter.embed(memory.content);

      this.logger.log(
        `Embedding generated (${embedding.length}).`,
      );
  ...
  async searchSimilar(
    ...
    const embedding =
      await this.aiRouter.embed(query);
