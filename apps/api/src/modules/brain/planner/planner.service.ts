...
export class PlannerService {
  ...
  async createPlan(
    ...
    try {
      ...
      const rawPlan = {
        ...
        goalId,
        strategy,
        status: BrainPlanStatus.PLANNED,
        priority: BrainPlanPriority.NORMAL,
        steps: rawSteps,
        createdAt: new Date(),
      };
  ...
  public async generateCorrectionArgs(
    ...
    try {
      const correctedArgs =
        extractAndParseJson<Record<string, unknown>>(jsonString);
...
Respond strictly with a JSON object containing a "steps" array. Do NOT wrap the JSON in markdown blocks (e.g. \`\`\`json). Do NOT use <think> or XML tags. Just return the raw JSON object.
    `.trim();
    ...
    this.logger.debug(
      `[Planner LLM Raw Response]: ${JSON.stringify(response.content)}`,
    );
    ...
    const parsed = extractAndParseJson<Record<string, unknown>>(jsonString);
