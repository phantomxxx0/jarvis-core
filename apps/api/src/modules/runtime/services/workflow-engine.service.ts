...
import { WorkflowDefinition, WorkflowExecutionDTO, WorkflowExecutionStatus, WorkflowStep } from '../contracts/workflow.dto';
import {
  ExecutionCompletedEvent,
  ExecutionFailedEvent,
  ExecutionTimedOutEvent
} from '../events/execution.events';
import { WorkflowValidationError } from './workflow-validator.service';
...
export class WorkflowEngineService {
  ...
  async submitWorkflow(userId: string, definition: WorkflowDefinition, name?: string): Promise<WorkflowExecutionDTO> {
    const initialState: Record<string, StepState> = {};
    for (const step of definition.steps) {
      initialState[step.id] = { status: 'PENDING' };
    }

    const [record] = await this.database.db.insert(workflowExecutions).values({
      userId,
      name,
    ...
    setImmediate(() => this.evaluateDAG(dto.id).catch(err => this.logger.error(`Error evaluating DAG for ${dto.id}`, err)));
    
    return dto;
  }

  private async withLock(workflowId: string, fn: () => Promise<void>): Promise<void> {
    const prev = this.workflowLocks.get(workflowId) || Promise.resolve();
    let release: () => void;
    const next = new Promise<void>(resolve => { release = resolve; });
    this.workflowLocks.set(workflowId, prev.then(() => next));
    try {
      await prev;
      await fn();
    } finally {
  ...
  private async evaluateDAG(workflowId: string): Promise<void> {
    await this.withLock(workflowId, async () => {
      const [record] = await this.database.db.select().from(workflowExecutions).where(eq(workflowExecutions.id, workflowId));
      if (!record) return;
      if (record.status !== 'PENDING' && record.status !== 'RUNNING') return;

      if (record.status === 'PENDING') {
        await this.updateWorkflowStatus(workflowId, WorkflowExecutionStatus.RUNNING, { startedAt: new Date() });
        record.status = 'RUNNING';
      }

      const definition = record.definition as unknown as WorkflowDefinition;
        ...
        if (stepState.status === 'PENDING') {
          ...
          if (step.dependencies && step.dependencies.length > 0) {
            for (const depId of step.dependencies) {
              ...
              if (!depState || (depState.status !== 'SUCCESS' && depState.status !== 'SKIPPED')) {
                canRun = false;
              }
          ...
          if (canRun) {
            ...
            if (step.condition) {
              try {
                ...
                const conditionResult = jsonLogic.apply(JSON.parse(step.condition), context);
                if (!conditionResult) {
                  state[step.id] = { status: 'SKIPPED' };
                  madeProgress = true;
                  continue;
                ...
                this.logger.warn(`Failed to evaluate condition for step ${step.id}: ${err}`);
                state[step.id] = { status: 'FAILED', error: 'Condition evaluation failed' };
                hasFailed = true;
                madeProgress = true;
                continue;
              }
            ...
            try {
              ...
              state[step.id] = { status: 'FAILED', error: 'Interpolation failed' };
              hasFailed = true;
              madeProgress = true;
              continue;
            }
            ...
            this.logger.log(`Submitting task for workflow ${workflowId}, step ${step.id}`);
            ...
            try {
              ...
              await this.database.db.update(taskExecutions)
                .set({ workflowExecutionId: workflowId, workflowStepId: step.id })
                .where(eq(taskExecutions.id, task.id));
                
              state[step.id].taskId = task.id;
            } catch (err) {
              this.logger.error(`Failed to submit task for step ${step.id}`, err);
              state[step.id] = { status: 'FAILED', error: 'Task submission failed' };
              hasFailed = true;
            }
          }
        }
      ...
      if (madeProgress) {
        await this.database.db.update(workflowExecutions)
          .set({ state, updatedAt: new Date() })
          .where(eq(workflowExecutions.id, workflowId));
      }

      if (hasFailed) {
        await this.updateWorkflowStatus(workflowId, WorkflowExecutionStatus.FAILED, { completedAt: new Date() });
      } else if (isComplete) {
        await this.updateWorkflowStatus(workflowId, WorkflowExecutionStatus.SUCCESS, { completedAt: new Date() });
      } else if (madeProgress) {
        // Loop again if we unblocked something synchronously (like SKIPPED)
        setImmediate(() => this.evaluateDAG(workflowId).catch(err => this.logger.error(err)));
      }
    });
  }
  ...
  async handleTaskSuccess(payload: ExecutionCompletedEvent) {
    await this.handleTaskCompletion(payload.executionId, 'SUCCESS', payload.output);
  }

  @OnEvent(ExecutionFailedEvent.name)
  async handleTaskFailed(payload: ExecutionFailedEvent) {
    await this.handleTaskCompletion(payload.executionId, 'FAILED', null, payload.error);
  }
  
  @OnEvent(ExecutionTimedOutEvent.name)
  async handleTaskTimeout(payload: ExecutionTimedOutEvent) {
    await this.handleTaskCompletion(payload.executionId, 'FAILED', null, { message: 'Timed out' });
  }

  private async handleTaskCompletion(taskId: string, status: 'SUCCESS' | 'FAILED', output?: any, error?: any) {
    // Find if task is part of a workflow
    const [task] = await this.database.db.select()
      .from(taskExecutions)
    ...
    await this.withLock(workflowId, async () => {
      const [workflow] = await this.database.db.select()
    ...
    this.evaluateDAG(task.workflowExecutionId).catch(err => this.logger.error(err));
  }

  private buildContext(state: Record<string, StepState>): any {
    const context: any = {};
  ...
  private interpolate(input: any, context: any): any {
  ...
  private async updateWorkflowStatus(id: string, status: WorkflowExecutionStatus, extraFields: any = {}) {
    await this.database.db.update(workflowExecutions)
      .set({ status, updatedAt: new Date(), ...extraFields })
      .where(eq(workflowExecutions.id, id));
  }

  private mapToDTO(record: any): WorkflowExecutionDTO {
    return {
      id: record.id,
