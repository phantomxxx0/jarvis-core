...
import { WorkflowDefinition, WorkflowExecutionStatus } from '../contracts/workflow.dto';
        ...
        where: jest.fn().mockResolvedValue([{ 
          ...
          definition: { 
            steps: [
              ...
              { id: 'step2', capabilityId: 'cap2', input: { arg: 2 }, dependencies: ['step1'] }
            ]
          }
        }]),
  ...
  describe('submitWorkflow', () => {
    it('should create workflow and start evaluation', async () => {
      const definition: WorkflowDefinition = {
        steps: [
          { id: 'step1', capabilityId: 'echo', input: 'hello' }
        ]
      };
  ...
  describe('evaluateDAG', () => {
    ...
    it('should skip step if condition evaluates to false', async () => {
      dbMock.db.where.mockResolvedValueOnce([{ 
        ...
        definition: { 
          steps: [
            { id: 'step1', capabilityId: 'cap1', input: {}, condition: '{"==": [1, 2]}' }
          ]
        }
      }]);

      await (service as any).evaluateDAG('wf-2');
      ...
      dbMock.db.where.mockResolvedValueOnce([{ 
        ...
        definition: { 
          steps: [
            ...
            { id: 'step2', capabilityId: 'cap2', input: { msg: 'hello ${step1.output.value}' }, dependencies: ['step1'] }
          ]
        }
      }]);

      await (service as any).evaluateDAG('wf-3');
      
      expect(orchestratorMock.submitTask).toHaveBeenCalledWith(
        'user-1', 'cap2', { msg: 'hello world' }, undefined, undefined
      );
    });
  });
});
